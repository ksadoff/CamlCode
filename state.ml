...
type clipboard = {clipboard_get : unit -> rope; clipboard_set: rope -> unit};
...
type state = {
  ...
  typing_area : typing_area;
  recent_commands : string list;
  clipboard : clipboard;
  current_name : File.t;
  first_visible : int;
  start_end_loc : (int * int);
  search_term : string;
...
let copy st =
  match unwrap_opt (st.current_name.selected_range) with
  | None -> st
  | Some (loc1, loc2) ->
  {st with clipboard with clipboard_set = sub current_name.contents loc1 loc2}
...
let paste st =
  let paste_text = to_string st.clipboard_get in
  let rope_before = sub st.current_name.contents 0 st.current_name.cursor in
  let rope_after = sub st.current_name.contents st.current_name.cursor (length st.current_name.contents) in
  let new_rope = st.clipboard_get in
  {st with current_name.contents = concat rope_before new_rope |> concat rope_after}
